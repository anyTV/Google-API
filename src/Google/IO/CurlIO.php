...
use Google\Exception\IOException;
use Google;
...
class CurlIO implements IO
{
    ...
    {
        $request = Google\Client::$auth->sign($request);
    ...
    public function setCachedRequest(HttpRequest $request)
    {
        ...
        if (CacheParser::isResponseCacheable($request)) {
            Google\Client::$cache->set($request->getCacheKey(), $request);
            return true;
        }
    ...
    public function getCachedRequest(HttpRequest $request)
    {
        ...
        return Google\Client::$cache->get($request->getCacheKey());
    }
