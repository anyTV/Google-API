 ...
 * under the License.
 */
use Google\IO\CacheParser;
use Google\IO\HttpRequest;

require_once 'io/CacheParser
class ApiCacheParserTest extends BaseTest {
  public function testIsResponseCacheable() {
    $resp = new HttpRequest('http://localhost', 'POST');
    $result = CacheParser::isResponseCacheable($resp);
    $this->assertFalse($result);


    // The response has expired, and we don't have an etag for
    // revalidation.
    $resp = new HttpRequest('http://localhost', 'GET');
    ...
    $resp = new HttpRequest('http://localhost', 'HEAD');
    $resp->setResponseHttpCode('200');
  ...
  public function testIsExpired() {
    ...
    $this->assertFalse(CacheParser::isExpired($resp));
    ...
    $this->assertTrue(CacheParser::isExpired($resp));
  ...
  public function testMustRevalidate() {
    ...
    $this->assertFalse(CacheParser::mustRevalidate($resp));
    ...
    $this->assertTrue(CacheParser::mustRevalidate($resp));
